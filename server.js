...
// 🏇 ৪টি রাজকীয় রেসিং ঘোড়ার অফিশিয়াল ডেটা ম্যাপ
const horseListPool = [
    { id: "RED_1", name: "লাল ১ নম্বর", weight: 25 },
    { id: "BLUE_2", name: "নীল ২ নম্বর", weight: 25 },
    { id: "GREEN_3", name: "সবুজ ৩ নম্বর", weight: 25 },
    { id: "YELLOW_4", name: "হলুদ ৪ নম্বর", weight: 25 }
];
...
// 🛫 ২. ৪-ঘোড়া রেস কোর এপিআই রাউট (POST Route - ৯৫% RTP গাণিতিক অ্যালগরিদম বর্ম লক ভাই ভাই!)
app.post('/api/derby-race', async (req, res) => {
    const { userId, amount, wallet, prediction } = req.body;
    const targetWallet = wallet || "main";
    const reqAmount = parseFloat(amount) || 50;
    const userPrediction = prediction || "RED_1"; // RED_1, BLUE_2, GREEN_3, YELLOW_4
    ...
    try {
        ...
        let selectedWinnerHorse, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল RTP ও সুষম ৪-ঘোড়া রেস র্যান্ডমাইজেশন লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            loopSafety++;
            
            // র্যান্ডম ওয়েটেড পুল থেকে বিজয়ী ঘোড়া বাছাই
            selectedWinnerHorse = horseListPool[Math.floor(Math.random() * horseListPool.length)];

            if (userPrediction === selectedWinnerHorse.id) {
                finalStatus = "win";
                winMultiplier = 3.50; // ঘোড়া রেসে জিতলে ৩.৫ গুণ কিলার প্রফিট ভাই ভাই!
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (adminTriggeredPrize) {
                // এডমিন জোর করে উইন বা লস ট্র্যাকিং কন্ডিশন
                ...
                // 🔒 ৯৫% আরটিপি প্রোটেকশন গেটওয়ে লক: স্বাভাবিক ট্র্যাকে রেস জেতার চান্স ৪৪% এ ব্যালেন্সড লক ভাই ভাই
                if (finalStatus === "win") {
                    if (Math.random() <= 0.44) {
                        isLoopActive = false;
                    }
        ...
        if (response.data && response.data.status === "ok") {
            ...
            return res.json({
                ...
                status: finalStatus,
                winAmount: winAmount,
                winnerHorse: selectedWinnerHorse.id,
                winnerName: selectedWinnerHorse.name
            });
        } else {
        ...
        console.error("Derby Racing Core Engine Error:", e.message);
...
// ১৬ নম্বর গেম ২৩০০০ এ চলছে, তাই ১৭ নম্বর রয়েল ডার্বি হর্স রেসিং গেম প্রজেক্টের স্বাধীন কাস্টম পোর্ট ২৪০০০ কড়া লক হলো ভাই ভাই!
